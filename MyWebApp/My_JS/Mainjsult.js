  ...
  d3.json('../data/countries.json', function (data) {
    ...
    for (i = 0; i < beer.length; i++) {
      ...
      if (x == Year && std == 'Super Premium' ) {
        ...
        for (var key1 = 1; key1 < keys.length; key1++) {
          ...
          if(x == Year && std == 'Super Premium' && beer[i].Country == cnt) {
            cases = beer[i].Cases;
    ...
    legend.onAdd = function (map) {
      var div = L.DomUtil.create('div', 'info legend'),
        grades = [0, 10000, 20000, 30000, 40000]
      labels = [];
